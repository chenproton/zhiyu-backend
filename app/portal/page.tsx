...
const features = [
  ...
  { label: "以学生能力为中心", active: false },
  { label: "以场景实践为载体", active: false },
  { label: "以跨专业融合为特征", active: false },
]
...
function PlatformCard({
  ...
  const cardContent = (
    <>
      <div className="flex items-start gap-2 mb-2">
        ...
        <h3 className="font-medium text-foreground text-xs leading-tight pt-1.5">
          {item.title}
        </h3>
      </div>
  ...
  if (url && enabled) {
    return (
      <a
        ...
        className="bg-card rounded-xl p-3.5 border border-border cursor-pointer hover:shadow-md hover:border-primary/30 transition-all block"
      >
        {cardContent}
      </a>
    )
  ...
  return (
    <div className="bg-card rounded-xl p-3.5 border border-border cursor-default opacity-70">
      {cardContent}
    </div>
  )
}
...
function MallBar({
  ...
  return (
    <div className="mt-2 bg-card rounded-xl px-5 py-3 border border-border flex items-center justify-between gap-4 hover:shadow-md hover:border-primary/30 transition-all">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="w-8 h-8 rounded-lg bg-pink-50 text-pink-500 border border-pink-100 flex items-center justify-center shrink-0">
          {getIcon(v2Mall.icon)}
        </div>
        <h3 className="font-medium text-xs text-foreground">{v2Mall.title}</h3>
        <div className="w-px h-3 bg-border hidden sm:block" />
        <p className="text-[11px] text-muted-foreground">{v2Mall.desc}</p>
      </div>
      {url && enabled ? (
        <a
          href={url}
...
export default function PortalHomePage() {
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-[#f5f7fa] flex flex-col pt-14">
      {/* Main Content */}
      <div className="flex-1 px-8 py-4">
        ...
        <div className="relative text-center mb-4">
          ...
          {/* Version Tag - top right of title */}
          <div className="absolute -right-1 top-0">
            <VersionTag version={version} onChange={setVersion} />
          </div>
          <div className="w-14 h-0.5 bg-primary mx-auto mb-3 rounded-full" />
          <div className="flex items-center justify-center gap-6">
            {features.map((feature, index) => (
              <div key={index} className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span className={`w-1.5 h-1.5 rounded-full ${feature.active ? "bg-primary" : "bg-muted"}`} />
                {feature.label}
              </div>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          {currentCategories.map((cat) => {
            ...
            return (
              <div key={cat.id} className="flex items-stretch gap-4 mb-3">
                {/* Left Category Label */}
                <div className="w-6 flex flex-col items-center justify-center py-2">
                  <div className={`w-0.5 flex-1 rounded-full ${cat.color} max-h-16`} />
                  <span
                    className="text-[10px] text-muted-foreground mt-1.5 whitespace-nowrap"
      ...
      {/* Footer */}
      <footer className="border-t border-border bg-background py-3 px-8">
