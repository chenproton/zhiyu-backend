...
function PlatformCard({
  ...
  if (url && enabled) {
    return (
      <a
        ...
        className="bg-card rounded-xl p-5 border border-border cursor-pointer hover:shadow-md hover:border-primary/30 transition-all block"
      >
        {cardContent}
      </a>
    )
  ...
  return (
    <div className="bg-card rounded-xl p-5 border border-border cursor-default opacity-70">
      {cardContent}
    </div>
  )
}
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-[#f5f7fa] flex flex-col pt-14">
      ...
      <div className="flex-1 px-8 py-10">
        ...
        <div className="text-center mb-10">
          <h1 className="text-3xl font-bold text-foreground mb-5">场景化数智教学服务体系</h1>
        ...
        <div className="max-w-6xl mx-auto">
          {categories.map((cat) => {
            const platform = platforms.find((p) => p.category === cat.id)
            if (!platform) return null

            return (
              <div key={cat.id} className="flex items-stretch gap-5 mb-5">
                {/* Left Category Label */}
